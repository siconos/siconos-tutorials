...
void doubleMixtureRHS(double* z, double* rhs){
    ...
    rhs[1] = energEvap(z[M2],z[x2],z[hv2]) - tempConduc_1_2(z[T1],z[T2]) - energCond(z[M1],z[x1],z[hl1]) ;    
    // Liquid Mixture Mass Balance Equation
    rhs[2] = massCond(z[M1],z[x1]) - massEvap(z[M2],z[x2]); 
    // Liquid Mixture Energy Balance Equation
    rhs[3] = energCond(z[M1],z[x1],z[hl1]) + tempConduc_1_2(z[T1],z[T2]) -energEvap(z[M2],z[x2],z[hv2]) + CTE.Qext;   
...
void display_model_state(double* z, int n, double h, int iteration){
    ...
    std::cout << std::endl;
    std::cout << "t =  " << ((iteration+1)*h) << " seconds " << std::endl;
    std::cout << "####\n" << std::endl;
}
...
int main(int argc, char *argv[]) {
  ...
  SP::FirstOrderLinearTIR LTIRBuckConverter_buck(
      new FirstOrderLinearTIR(Int_C_buck, Int_B_buck));
  ...
  SP::TimeStepping StratBuckConverter(
      new TimeStepping(NSDSBuckConverter, TiDisc, OSI_LSBuckConverter, LCP_BuckConverter));
  ...
  if ((error = ioMatrix::compareRefFile(dataPlot, "BuckConverter.ref", eps)) >= 0.0 &&
      error > eps)
    return 1;

...
int main(int argc, char* argv[]) {
  try {
    ...
    SP::Relation relation(
        new LagrangianScleronomousR("SimplePendulumBilateralConstraintPlugin:h0",
    ...
    SP::SiconosVector _qddot = simplependulum->acceleration();
    ;  // pointer points to the acceleration
    ...
    while ((EDscheme->hasNextEvent()) && (k < N)) {
      ...
      if (NSEvent) {
        ...
        const SiconosVector& _qdotMemory =
    ...
    if ((error = ioMatrix::compareRefFile(DataPlot, "SimplePendulumED_Lsodar.ref", eps)) >=
            0.0 &&
        error > eps)
      return 1;
    return 0;

...
int main(int argc, char* argv[]) {
  try {
    ...
    SP::NonSmoothLaw nslaw(
        new NewtonImpactFrictionNSL(eN, eT, mu, 2));  // EqualityConditionNSL NewtonImpactNSL
    SP::Relation relation(
        new LagrangianScleronomousR("FourBarClearancePlugin:g1", "FourBarClearancePlugin:W1"));
    SP::Interaction inter(new Interaction(nslaw, relation));

    SP::NonSmoothLaw nslaw1(
        new NewtonImpactFrictionNSL(eN, eT, mu, 2));  // EqualityConditionNSL NewtonImpactNSL
    SP::Relation relation1(
        new LagrangianScleronomousR("FourBarClearancePlugin:g2", "FourBarClearancePlugin:W2"));
    SP::Interaction inter1(new Interaction(nslaw1, relation1));

    SP::NonSmoothLaw nslaw2(
        new NewtonImpactFrictionNSL(eN, eT, mu, 2));  // EqualityConditionNSL NewtonImpactNSL
    SP::Relation relation2(
        new LagrangianScleronomousR("FourBarClearancePlugin:g3", "FourBarClearancePlugin:W3"));
    SP::Interaction inter2(new Interaction(nslaw2, relation2));

    // SP::NonSmoothLaw nslaw2(new EqualityConditionNSL(e1)); //EqualityConditionNSL
    // NewtonImpactNSL SP::Relation relation2(new
    // LagrangianScleronomousR("FourBarClearancePlugin:g3", "FourBarClearancePlugin:W3"));
    // SP::Interaction inter2(new Interaction(nslaw2, relation2));

    // SP::NonSmoothLaw nslaw3(new EqualityConditionNSL(e1)); //EqualityConditionNSL
    // NewtonImpactNSL SP::Relation relation3(new
    // LagrangianScleronomousR("FourBarClearancePlugin:g4", "FourBarClearancePlugin:W4"));
    ...
    SP::OneStepNSProblem impact(new FrictionContact(
        2, SICONOS_FRICTION_2D_NSGS)); /*,SICONOS_FRICTION_2D_ENUM
                                        SICONOS_FRICTION_2D_LEMKE notworking //
                                        SICONOS_FRICTION_2D_PGS=>working 0.75numbers::pi */
    ...
    SP::TimeSteppingCombinedProjection s(
        new TimeSteppingCombinedProjection(FourBarIdeal, t, OSI, impact, position, 2));
    ...
    dataPlot(0, 36) = 0.5 * ((*v)(0) - 6.0 * 0.75 * PI * cos(0.75 * PI * h)) *
                          ((*v)(0) - 6.0 * 0.75 * PI * cos(0.75 * PI * h)) +
                      0.5 * 3000.0 * ((*q)(0) - 6.0 * sin(0.75 * PI * h)) *
                          ((*q)(0) - 6.0 * sin(0.75 * PI * h)) +
                      0.5 * 10.0 * ((*v)(0) - 6.0 * 0.75 * PI * cos(0.75 * PI * h)) *
                          ((*q)(0) - 6.0 * sin(0.75 * PI * h));
    ...
    while (s->hasNextEvent()) {
      ...
      if (((k % 500) == 0) && (k != 0)) {
        ...
        dataPlot(kk, 36) = 0.5 * ((*v)(0) - 6.0 * 0.75 * PI * cos(0.75 * PI * tt)) *
                               ((*v)(0) - 6.0 * 0.75 * PI * cos(0.75 * PI * tt)) +
                           0.5 * 3000.0 * ((*q)(0) - 6.0 * sin(0.75 * PI * tt)) *
                               ((*q)(0) - 6.0 * sin(0.75 * PI * tt)) +
                           0.5 * 10.0 * ((*v)(0) - 6.0 * 0.75 * PI * cos(0.75 * PI * tt)) *
                               ((*q)(0) - 6.0 * sin(0.75 * PI * tt));

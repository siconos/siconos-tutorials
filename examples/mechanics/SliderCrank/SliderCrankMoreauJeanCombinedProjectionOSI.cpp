...
int main(int argc, char *argv[]) {
  try {
    ...
    // --- Simulation ---
    // ----------------
    SP::MoreauJeanCombinedProjectionOSI OSI(
        new MoreauJeanCombinedProjectionOSI(0.5));
    ...
    SP::TimeSteppingCombinedProjection s(new TimeSteppingCombinedProjection(
    ...
    int elapsed =
        std::chrono::duration_cast<std::chrono::milliseconds>(end - start)
            .count();
    cout << "Computation time : " << elapsed << " ms" << k << endl;
    // --- Output files ---

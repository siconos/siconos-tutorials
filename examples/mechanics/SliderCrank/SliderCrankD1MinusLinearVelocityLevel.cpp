...
int main(int argc, char *argv[]) {
  try {
    ...
    // --- Simulation ---
    // ----------------
    SP::D1MinusLinearOSI OSI(
        new D1MinusLinearOSI(D1MinusLinearOSI::halfexplicit_velocity_level));
    SP::TimeDiscretisation t(new TimeDiscretisation(t0, h));
    SP::OneStepNSProblem impact(new LCP());
    SP::OneStepNSProblem force(new LCP());

    SP::TimeSteppingD1Minus s(
        new TimeSteppingD1Minus(sliderWithClearance, t, 2));
    ...
    // not yet allocated
    // dataPlot(k, 31) = ( inter1->lambdaMemory(2).getSiconosVector(0) )(0) ; //
    // lambda1old dataPlot(k, 32) = (
    // inter2->lambdaMemory(2).getSiconosVector(0) )(0) ; // lambda1old
    // dataPlot(k, 33) = ( inter3->lambdaMemory(2).getSiconosVector(0) )(0) ; //
    // lambda1old dataPlot(k, 34) = (
    // inter4->lambdaMemory(2).getSiconosVector(0) )(0) ; // lambda1old
    ...
    while ((s->hasNextEvent())) {
      ...
      dataPlot(k, 26) = 0;
      dataPlot(k, 27) =
          (*inter1->lambda(2))(0); // lambda1_{k+1}^- (nonimpulsive force)
      dataPlot(k, 28) = (*inter2->lambda(2))(0); // lambda1_{k+1}^-
      dataPlot(k, 29) = (*inter3->lambda(2))(0); // lambda1_{k+1}^-
      dataPlot(k, 30) = (*inter4->lambda(2))(0); // lambda1_{k+1}^-

      dataPlot(k, 31) =
          (inter1->lambdaMemory(2).getSiconosVector(0))(0); // lambda1_k^+
      dataPlot(k, 32) =
          (inter2->lambdaMemory(2).getSiconosVector(0))(0); // lambda2_k^+
      dataPlot(k, 33) =
          (inter3->lambdaMemory(2).getSiconosVector(0))(0); // lambda3_k^+
      dataPlot(k, 34) =
          (inter4->lambdaMemory(2).getSiconosVector(0))(0); // lambda4_k^+
    ...
    if ((error = ioMatrix::compareRefFile(
             dataPlot, "SliderCrankD1MinusLinearOSIVelocityLevel.ref", eps)) >=
            0.0 &&
        error > eps)
      return 1;
